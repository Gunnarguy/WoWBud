...
struct TalentCalculatorView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            VStack(spacing: 8) {
                ...
                .padding(.horizontal)
                .onChange(of: characterLevel) { _ in
                ...
                ForEach(0..<6) { tier in
                    HStack(spacing: 8) {
                        ...
                        ForEach(0..<4) { column in
                            ...
                            if let talent = spec.talents.first(where: { $0.tier == tier && $0.column == column }) {
        ...
        return VStack(spacing: 4) {
            ...
            ZStack {
                RoundedRectangle(cornerRadius: 6)
                    .fill(talentBackgroundColor(talent: talent, currentPoints: currentPoints, specIndex: specIndex))
                    .frame(width: 52, height: 52)
                    .overlay(
                        RoundedRectangle(cornerRadius: 6)
                        ...
                        ForEach(savedBuilds) { build in
                            Button(action: {
                                ...
                                VStack(alignment: .leading, spacing: 4) {
                                    ...
                                    Text("\(build.className) (\(build.points[0])/\(build.points[1])/\(build.points[2]))")
                                        .font(.subheadline)
                                        .foregroundColor(.secondary)
                                }
                                .padding(.vertical, 4)
        ...
        classes = [
            ClassData(
                ...
                specs: [
                    ...
                    ClassData.Spec(name: "Protection", talents: createTalents(specIndex: 2, classID: 1))
                ]
            ),
            ClassData(
                id: 2,
                ...
                specs: [
                    ...
                    ClassData.Spec(name: "Protection", talents: createTalents(specIndex: 1, classID: 2)),
                    ClassData.Spec(name: "Retribution", talents: createTalents(specIndex: 2, classID: 2))
                ]
            ),
            ClassData(
                id: 3,
                ...
                powerType: "Mana",
                specs: [
                    ClassData.Spec(name: "Beast Mastery", talents: createTalents(specIndex: 0, classID: 3)),
                    ClassData.Spec(name: "Marksmanship", talents: createTalents(specIndex: 1, classID: 3)),
                    ClassData.Spec(name: "Survival", talents: createTalents(specIndex: 2, classID: 3))
                ]
            ),
            ClassData(
                id: 4,
                ...
                powerType: "Energy",
                specs: [
                    ClassData.Spec(name: "Assassination", talents: createTalents(specIndex: 0, classID: 4)),
                    ClassData.Spec(name: "Combat", talents: createTalents(specIndex: 1, classID: 4)),
                    ClassData.Spec(name: "Subtlety", talents: createTalents(specIndex: 2, classID: 4))
                ]
            ),
            ClassData(
                id: 5,
                ...
                powerType: "Mana",
                specs: [
                    ClassData.Spec(name: "Discipline", talents: createTalents(specIndex: 0, classID: 5)),
                    ClassData.Spec(name: "Holy", talents: createTalents(specIndex: 1, classID: 5)),
                    ClassData.Spec(name: "Shadow", talents: createTalents(specIndex: 2, classID: 5))
                ]
            ),
            ClassData(
                id: 7,
                ...
                powerType: "Mana",
                specs: [
                    ClassData.Spec(name: "Elemental", talents: createTalents(specIndex: 0, classID: 7)),
                    ClassData.Spec(name: "Enhancement", talents: createTalents(specIndex: 1, classID: 7)),
                    ClassData.Spec(name: "Restoration", talents: createTalents(specIndex: 2, classID: 7))
                ]
            ),
            ClassData(
                id: 8,
                ...
                powerType: "Mana",
                specs: [
                    ClassData.Spec(name: "Arcane", talents: createTalents(specIndex: 0, classID: 8)),
            ...
            ClassData(
                ...
                powerType: "Mana",
                specs: [
                    ClassData.Spec(name: "Affliction", talents: createTalents(specIndex: 0, classID: 9)),
                    ClassData.Spec(name: "Demonology", talents: createTalents(specIndex: 1, classID: 9)),
                    ClassData.Spec(name: "Destruction", talents: createTalents(specIndex: 2, classID: 9))
                ]
            ),
            ClassData(
                id: 11,
                ...
                powerType: "Mana",
                specs: [
                    ClassData.Spec(name: "Balance", talents: createTalents(specIndex: 0, classID: 11)),
                    ClassData.Spec(name: "Feral Combat", talents: createTalents(specIndex: 1, classID: 11)),
                    ClassData.Spec(name: "Restoration", talents: createTalents(specIndex: 2, classID: 11))
                ]
            )
        ]
        ...
        for tier in 0..<6 {
            ...
            for column in 0..<columnsInThisTier {
                ...
                if tier >= 2 && Bool.random() {
                    ...
                    prerequisiteID = (classID * 1000) + (specIndex * 100) + (prereqTier * 10) + prereqColumn
                }
    ...
    private func canUnlearnTalent(talent: ClassData.Talent, specIndex: Int) -> Bool {
        ...
        guard let currentPoints = selectedTalents[specIndex]?[talent.id], currentPoints > 0 else { return false }
        
        // Get class and spec data
        guard let currentClass = classes.first(where: { $0.id == selectedClassID }),
              let spec = currentClass.specs[safe: specIndex] else { return false }
        ...
        let talentsByTier = Dictionary(grouping: selectedTalents[specIndex]?.keys ?? []) { talentID in
            spec.talents.first(where: { $0.id == talentID })?.tier ?? 0
        }
        ...
        if pointTier < highestTier {
            ...
            let pointsInThisTier = talentsByTier[pointTier]?.compactMap { selectedTalents[specIndex]?[$0] }.reduce(0, +) ?? 0
    ...
    private func talentBackgroundColor(talent: ClassData.Talent, currentPoints: Int, specIndex: Int) -> Color {
